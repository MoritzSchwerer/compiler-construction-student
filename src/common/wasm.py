...
class WasmImport:
    ...
    def render(self):
        return mkNamedSeq(
            "import", SExpStr(self.module), SExpStr(self.name), self.desc.render()
        )
...
class WasmData:
    ...
    def render(self) -> SExp:
        return mkNamedSeq(
            "data", SExpId(f"(i32.const {self.start})"), SExpStr(self.content)
        )
...
class WasmFunc:
    ...
    def render(self) -> SExp:
        params = [
            mkNamedSeq("param", i.render(), renderValtype(t)) for (i, t) in self.params
        ]
        res = []
        if self.result:
            res = [mkNamedSeq("result", renderValtype(self.result))]
        locals = [
            mkNamedSeq("local", i.render(), renderValtype(t)) for (i, t) in self.locals
        ]
        instrs = [x.render() for x in self.instrs]
        return mkNamedSeq("func", self.id.render()).append(
            params + res + locals + instrs
        )
...
class WasmInstrIf:
    ...
    def render(self) -> SExp:
        res = (
            ...
            else [SExpSeq([SExpId("result"), (SExpId(self.resultType))])]
        )
...
class WasmInstrLoop:
    ...
    def render(self) -> SExp:
        return SExpBlock.singleItem(
            "loop", [self.label.render()] + [i.render() for i in self.body]
        )
...
class WasmInstrBlock:
    ...
    def render(self) -> SExp:
        ...
        if self.result:
            res = [mkNamedSeq("result", renderValtype(self.result))]
        return SExpBlock.singleItem(
            "block", [self.label.render()] + res + [i.render() for i in self.body]
        )
